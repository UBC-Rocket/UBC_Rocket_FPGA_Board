module MULT (clk, areset, start, a, b, q, finish);
    ...
    always_comb begin
        ...
        e_c1 = (exp_a == 8'd0 ? 10'd0 : {2'b00, exp_a})
             + (exp_b == 8'd0 ? 10'd0 : {2'b00, exp_b})
             - 10'd127;
    end
    ...
    // -------------------------
    // Stage 2: 24x24 multiply
    // -------------------------
    // (* use_dsp = "no" *)  // <- uncomment if you need to forbid DSP usage (tool-specific)
    ...
    always_comb begin
        ...
        if (a_zero_s2 || b_zero_s2) begin
            ...
            if (e >= 10'd255) begin
                ...
                exp_norm      = 8'd0;   // underflow -> treat as 0 (simple)
                is_zero_result= 1'b1;
            end else begin
                exp_norm = e[7:0];
            end
            ...
            if (is_zero_result) begin
                ...
                q_c3 = {sign_res_s2, 8'hFF, 23'd0}; // ±Inf
            end else begin
    ...
    assign q      = q_s3;
    assign finish = valid_s3;   // fires 4 cycles after 'start'
